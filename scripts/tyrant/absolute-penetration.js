...
function update_tier1() {
    ...
    gi("display_tier1").textContent = "patk=" + patk.toString() + " * tier1_total_modifiers=" + tier1_mod.toString() + " = " + tier1.toString()
}
...
function update_tier2() {
    ...
    usable_matk = three_decimals(matk * 0.71)
    ...
    tier2_mod = race_mod * pvp_mod
    tier2_mod = three_decimals(tier2_mod)
    tier2_base = tier1 + stat_atk + usable_matk
    ...
    gi("ratio_matk").textContent = three_decimals(usable_matk / tier2_base)

    gi("comp_attacker-stats").textContent = stat_atk
    gi("comp_race-mod").textContent = race_mod
    gi("comp_pvp-mod").textContent = pvp_mod
    update_span_class("dup_matk", matk)
    gi("comp_tier2").textContent = tier2
    gi("display_tier2").textContent = "(tier1=" + tier1.toString() + " + stat_atk=" + stat_atk.toString() + 
        " + (matk=" + matk.toString() + " * 0.71)=" + usable_matk.toString() + ")=" + 
        tier2_base.toString() + " * tier2_total_modifiers=" + tier2_mod.toString() + " = " + tier2.toString()
}

function update_tier3() {
    update_tier2()
    tier2 = parseFloat(gi("comp_tier2").textContent)
    luk_bonus = parseFloat(giv("i_luk-passive-lvl")) * 3.0 * parseInt(giv("i_attacker-luk"))
    ...
    tier3_base = tier2 + luk_bonus
    tier3_mod = three_decimals(def_mod * phys_mod)
    tier3 = three_decimals(tier3_base * tier3_mod)

    prev_tier_ratio = three_decimals(tier2 / tier3_base)
    gi("ratio_luk-bonus").textContent = three_decimals(luk_bonus / tier3_base)
    ...
    gi("ratio_matk").textContent =  three_decimals(gitc_float("ratio_matk") * prev_tier_ratio)

    gi("comp_luk-bonus").textContent = luk_bonus
    ...
    gi("display_tier3").textContent = "(tier2=" + tier2.toString() + " + luk_bonus=" + luk_bonus.toString() + 
        ")=" + tier3_base + " * tier3_total_modifiers=" + tier3_mod.toString() + " = " + tier3.toString()
}

function calc_abs_pen_skill_mod() {
    crackshot_lvl = parseFloat(giv("i_crackshot-lvl"))
    crackshot_mod = (crackshot_lvl * 0.60) + 12.00
    abs_pen_lvl = parseFloat(giv("i_abs-pen-lvl"))
    abs_pen_mod = 1 + (abs_pen_lvl * 0.03)

    assault_term_mod = 1 + (parseFloat(giv("i_weapon-refine")) * 0.02)
    gi("comp_assault-term-mod").textContent = three_decimals(assault_term_mod)

    aesir_mod = 1 + parseFloat(giv("i_aesir-mod"))
    distance_mod = 1 + (parseFloat(giv("i_distance-metres")) * 0.07)
    charging_mod = 1 + (parseInt(giv("i_charging-time")) * 0.50)

    def_per_bonus = (1 + parseFloat(giv("i_defender-def-per") / 100))
    raw_def_bonus = def_per_bonus * parseFloat(giv("i_defender-raw-def"))
    def_bonus_mod = raw_def_bonus / 500
    gi("comp_def-bonus-mod").textContent = def_bonus_mod

    abs_pen_skill_mod = (crackshot_mod * abs_pen_mod * assault_term_mod * aesir_mod) + def_bonus_mod
    abs_pen_skill_mod = three_decimals(abs_pen_skill_mod * distance_mod * charging_mod)
    return abs_pen_skill_mod
}
...
function update_tier4() {
    ...
    skill_mod_per = calc_abs_pen_skill_mod()
    enemy_vit = parseInt(giv("i_defender-vit"))
    vit_reduct = enemy_vit * 2

    tier4_base = three_decimals(tier3 + refine_atk)
    tier4_mod = three_decimals(refine_reduct * range_mod * skill_mod_per)
    tier4 = three_decimals(tier4_base * tier4_mod)
    tier4 = three_decimals(tier4 - vit_reduct)

    prev_tier_ratio = three_decimals(tier3 / tier4_base)
    gi("ratio_refine-atk").textContent = three_decimals(refine_atk / tier4_base)
    gi("ratio_patk").textContent = three_decimals(gitc_float("ratio_patk") * prev_tier_ratio)
    gi("ratio_stat-atk").textContent = three_decimals(gitc_float("ratio_stat-atk") * prev_tier_ratio)
    gi("ratio_matk").textContent =  three_decimals(gitc_float("ratio_matk") * prev_tier_ratio)
    gi("ratio_luk-bonus").textContent = three_decimals(gitc_float("ratio_luk-bonus") * prev_tier_ratio)

    update_span_class("dup_refine-atk", refine_atk)
    update_span_class("dup_defender-vit", defender_vit)
    gi("comp_range-mod").textContent = range_mod
    gi("comp_abs-pen-mod").textContent = skill_mod_per
    gi("comp_refine-reduct").textContent = refine_reduct
    gi("comp_tier4").textContent = tier4
    gi("display_tier4").textContent = "(tier3=" + tier3.toString() + " + refine_atk=" + refine_atk.toString() + ")=" + tier4_base.toString() + 
        " * tier4_total_modifiers=" + tier4_mod.toString() +
        ") - vit_reduct=" + vit_reduct.toString() + " = " + tier4.toString()
}
...
function update_tier5() {
    update_tier4()
    tier4 = parseFloat(gi("comp_tier4").textContent)
    ...
    tier5_base = three_decimals(tier4 + comp_true_dmg)
    ...
    tier5_mod = skill_dmg_mod * final_dmg_mod * status_dmg_mod
    tier5 = tier5_base * tier5_mod
    tier5 = three_decimals(tier5)

    prev_tier_ratio = three_decimals(tier4 / tier5_base)
    gi("ratio_true-dmg").textContent = three_decimals(comp_true_dmg / tier5_base * 100)
    ...
    gi("ratio_matk").textContent =  three_decimals(gitc_float("ratio_matk") * prev_tier_ratio * 100)
    gi("ratio_luk-bonus").textContent = three_decimals(gitc_float("ratio_luk-bonus") * prev_tier_ratio * 100)
    ...
    gi("comp_tier5").textContent = tier5

    update_span_class("dup_tier5", parseInt(tier5).toLocaleString("en-US"))
    gi("display_tier5").textContent = "(tier4=" + tier4.toString() + " + true_dmg=" + comp_true_dmg.toString() + 
        ")=" + tier5_base + " * tier5_total_modifiers=" + tier5_mod.toString() + " = " + tier5.toString()
}
